...
test_switch_to_parent_frame() {
    expect_http_response \
        "switch_to_parent_frame http://localhost:4444 aSession {"foo":"bar"}" \
...
test_get_window_rect() {
    expect_http_response \
        "get_window_rect http://localhost:4444 $session_id"\
        ""
}

test_set_window_rect() {
    expect_http_response \
        "set_window_rect http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_maximize_window() {
    expect_http_response \
        "maximize_window http://localhost:4444 $session_id"\
        ""
}

test_minimize_window() {
    expect_http_response \
        "minimize_window http://localhost:4444 $session_id"\
        ""
}

test_fullscreen_window() {
    expect_http_response \
        "fullscreen_window http://localhost:4444 $session_id"\
        ""
}

test_find_element() {
    expect_http_response \
        "find_element http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_find_elements() {
    expect_http_response \
        "find_elements http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_find_element_from_element() {
    expect_http_response \
        "find_element_from_element http://localhost:4444 $session_id $element {\"foo\":\"bar\"}"\
        ""
}

test_find_elements_from_element() {
    expect_http_response \
        "find_elements_from_element http://localhost:4444 $session_id $element {\"foo\":\"bar\"}"\
        ""
}

test_find_element_from_shadow_root() {
    expect_http_response \
        "find_element_from_shadow_root http://localhost:4444 $session_id $shadow {\"foo\":\"bar\"}"\
        ""
}

test_find_elements_from_shadow_root() {
    expect_http_response \
        "find_elements_from_shadow_root http://localhost:4444 $session_id $shadow {\"foo\":\"bar\"}"\
        ""
}

test_get_active_element() {
    expect_http_response \
        "get_active_element http://localhost:4444 $session_id"\
        ""
}

test_get_element_shadow_root() {
    expect_http_response \
        "get_element_shadow_root http://localhost:4444 $session_id $element"\
        ""
}

test_is_element_selected() {
    expect_http_response \
        "is_element_selected http://localhost:4444 $session_id $element"\
        ""
}

test_get_element_attribute() {
    expect_http_response \
        "get_element_attribute http://localhost:4444 $session_id $element id"\
        ""
}

test_get_element_property() {
    expect_http_response \
        "get_element_property http://localhost:4444 $session_id $element textContent"\
        ""
}

test_get_element_css_value() {
    expect_http_response \
        "get_element_css_value http://localhost:4444 $session_id $element color"\
        ""
}

test_get_element_text() {
    expect_http_response \
        "get_element_text http://localhost:4444 $session_id $element"\
        ""
}

test_get_element_tag_name() {
    expect_http_response \
        "get_element_tag_name http://localhost:4444 $session_id $element"\
        ""
}

test_get_element_rect() {
    expect_http_response \
        "get_element_rect http://localhost:4444 $session_id $element"\
        ""
}

test_is_element_enabled() {
    expect_http_response \
        "is_element_enabled http://localhost:4444 $session_id $element"\
        ""
}

test_get_computed_role() {
    expect_http_response \
        "get_computed_role http://localhost:4444 $session_id $element"\
        ""
}

test_get_computed_label() {
    expect_http_response \
        "get_computed_label http://localhost:4444 $session_id $element"\
        ""
}

test_element_click() {
    expect_http_response \
        "element_click http://localhost:4444 $session_id $element"\
        ""
}

test_element_clear() {
    expect_http_response \
        "element_clear http://localhost:4444 $session_id $element"\
        ""
}

test_element_send_keys() {
    expect_http_response \
        "element_send_keys http://localhost:4444 $session_id $element {\"foo\":\"bar\"}"\
        ""
}

test_get_page_source() {
    expect_http_response \
        "get_page_source http://localhost:4444 $session_id"\
        ""
}

test_execute_script() {
    expect_http_response \
        "execute_script http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_execute_async_script() {
    expect_http_response \
        "execute_async_script http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_get_all_cookies() {
    expect_http_response \
        "get_all_cookies http://localhost:4444 $session_id"\
        ""
}

test_get_named_cookie() {
    expect_http_response \
        "get_named_cookie http://localhost:4444 $session_id "foo""\
        ""
}

test_add_cookie() {
    expect_http_response \
        "add_cookie http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_perform_actions() {
    expect_http_response \
        "perform_actions http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}

test_release_actions() {
    expect_http_response \
        "release_actions http://localhost:4444 $session_id"\
        ""
}

test_dismiss_alert() {
    expect_http_response \
        "dismiss_alert http://localhost:4444 $session_id"\
        ""
}

test_accept_alert() {
    expect_http_response \
        "accept_alert http://localhost:4444 $session_id"\
        ""
}

test_get_alert_text() {
    expect_http_response \
        "get_alert_text http://localhost:4444 $session_id"\
        ""
}

test_send_alert_text() {
    expect_http_response \
        "send_alert_text http://localhost:4444 $session_id {\"foo\":\"bar\"}"\
        ""
}
